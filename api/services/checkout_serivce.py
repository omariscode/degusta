...
class CheckoutService:
    ...
    def process_checkout(user, data):
        ...
        for item in items:
            ...
            product = product_model.Product.objects.select_for_update().filter(pk=product_id).first()
            if not product:
        ...
        for product, qty in product_objs.values():
            order_model.OrderItem.objects.create(order=order, product=product, qty=qty, price=product.price)
            product.stock -= qty
            product.save()
        ...
        send_sms(['+18777804236'], f"Novo pedido de {invoice.billing_name}, local de entrega: {order.delivery_address}, com o pagamento total de {order.total}Kz.")
